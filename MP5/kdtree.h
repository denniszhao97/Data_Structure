...
class KDTree
{
  ...
  private:
   ...
   int quickSelect(int start, int end, int k, int dimension);
   int partition(int start, int end, int index, int dimension);
   void Destroy(KDTreeNode* root_);
   void findNearestNeighborHelper(const Point<Dim>& query,Point<Dim>& result, int start, int end, int dimension, double& distance, bool& exists) const;
};
